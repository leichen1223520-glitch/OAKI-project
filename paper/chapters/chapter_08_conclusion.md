...
基于10项常规检测参数（进出水COD/TN/NH₃-N、进水SS、好氧区MLSS、月度总电耗和污泥产量）的FPCM-L3模型，在两个基于文献参数构建的典型情景（南方中型AAO处理厂和北方大型AAO处理厂，第七章7.1—7.5节）的理论分析中，年度碳排放估算相对误差分别约+8%～+12%，Nash-Sutcliffe效率系数分别约0.83和0.79，满足工程碳核查所要求的±15%/NSE≥0.70验收标准。在深圳市46座处理厂的实证应用中，Scope 2（能耗）单位强度（0.211 kgCO₂eq/m³，直接由实测电耗计算）与全国文献均值（Wang等，2021 [143]：0.212 kgCO₂eq/m³）几乎完全一致，从实际运营数据层面独立验证了FPCM能耗模块（M3/M4）的参数合理性。这些结果共同支撑了"轻量化数据策略可行"的核心命题，为国内超过85%不具备在线气体监测能力的中小型污水处理厂提供了可操作的碳排放核算路径。
...
M2模型DO路径切换函数的物理合理性来自多项独立文献的交叉验证：Ahn等（2010 [23]）在美国全规模处理厂中证实低DO激活AOB反硝化路径；Wunderlin等（2012 [20]）通过¹⁵N同位素示踪证实DO=0.5～1.5 mg/L区间为路径B最活跃区；Ribera-Guardia等（2014 [28]）的全规模实验确认DO=2.0～2.5 mg/L时N₂O最低；Wang等（2019 [26]）在12座北方处理厂的长期测量中报告了同样的非单调关系。这些文献数据共同支撑了M2的以下工程结论：**将好氧区DO稳定控制在1.8～2.2 mg/L，预期可比DO大幅波动（均值2.0 mg/L但σ>0.4 mg/L）的工况减少N₂O约15%，折算碳减排约124 tCO₂eq/年（以情景A规模8万m³/d估算）**，且可同步节省约5%电耗（Yang等，2020 [74]）。该结论不依赖于特定实测厂的数据，而是从机理文献与模型参数逻辑共同推导得出。
...
### 结论六：中国城市污水处理中低C/N进水是N₂O风险的系统性来源

情景B（北方，进水COD/TN=5.18，来源于Wang等（2021 [143]）和Wan等（2016 [25]）北方城市典型值）的N₂O排放因子比情景A（南方，COD/TN=7.38）高出约41%，N₂O占全厂碳排放比例高出约6个百分点。这一差异通过M2-denit的C/N修正函数（$g(COD/TN)$）捕捉，与Pijuan等（2014 [33]）、Wan等（2016 [25]）和Wang等（2021 [143]）的文献实测吻合。

**特别重要的是**，本研究深圳46厂实测数据（XLS，276个厂-月数据点）显示，20.3%的数据点进水COD/TN<6.0，涉及18座不同厂站；这些厂包括沙田（均值4.80）、鹅埠（5.09）、沙井三期（5.10）等。这一实测发现表明，低C/N问题并非仅存在于北方城市——即使是南方经济发达的深圳，也有相当比例的处理厂面临碳源不足导致N₂O增高的风险。**当进水COD/TN<6.0时，IPCC Tier 1方法对N₂O的低估幅度预计将显著高于COD/TN正常工况（比值>7）时**，M2-denit的C/N修正项在这些情境下发挥不可省略的补偿作用。
...
**局限一：典型情景验证缺乏实测处理厂原始数据的直接校准**  
本研究的FPCM精度边界分析（7.1—7.5节）基于文献参数构建的典型情景，非特定实测设施的逐月运营台账；FPCM的实证应用（7.7节）虽使用了深圳46厂真实数据，但受限于数据类型（无独立的气体排放通量测量），Scope 1的直接验证依赖于模型计算而非独立测量值。未来研究应在具备N₂O/CH₄气体通量测定的实际处理厂开展12个月以上的FPCM验证，以获得完整的实测-预测对比。此外，目前分析中未涵盖倒置AAO、A²O、UCT等常见变型，以及处理规模<2万m³/d的小型处理厂——这些类型在全国数量众多，MCF和N₂O排放因子可能与当前先验分布存在差异。
