...
### 7.7.8 数据扩展计划：2026年4–6月

#### 7.7.8.1 当前数据局限

本研究深圳实证应用目前覆盖 2025年10月至2026年3月共6个月，涵盖秋、冬、春三季（深圳气候分区下，10月为秋季，11月至翌年2月为冬旱季，3月为春季）。这一数据范围有以下几点主要局限：

**（1）缺少春末夏初高温高水量数据**

深圳的气候特征决定了4–6月（华南前汛期/梅雨季）是全年水量最大的阶段之一，进水水量可比2月增加30%～50%，污水温度升至22–27°C，生物系统活性处于年度峰值。缺少这一阶段数据，导致：
- 碳排放年化估算（"6个月×2"）缺少对高温高水量季节的直接实测依据；
- 无法验证M1（CH₄）温度修正函数 $\theta_T^{T-20}$ 在高温区（T>25°C）的行为；
- 无法分析梅雨季进水大量稀释对进水C/N比的影响（及其对路径C的N₂O效应）。

**（2）季节性参数变化的全年覆盖不完整**

药耗（碳源、除磷药剂）的季节性规律在6个月数据中已初步显现（冬高夏低），但缺少4–6月的实测数据无法确认"碳源需求在何月开始下降""除磷效率的春季回升幅度"等关键规律。

#### 7.7.8.2 扩展后预期的新发现

一旦2026年4–6月数据纳入分析，基于现有数据的趋势外推，预期可观察到以下新发现：

**（1）单位碳排放的全年"V型"或"U型"季节曲线**

现有数据（10月至3月）显示单位碳排放从10月（0.4895）升至1月（0.5322），然后在3月回落至0.5049。随着4–6月水温上升（22→27°C）、处理水量增大、生物活性增强，单位碳排放预计进一步降低至0.46–0.49 kg/m³区间，形成全年"低（夏秋）→高（冬）→低（夏）"的倒U型或W型曲线。

**（2）碳源单耗的显著下降**

2025年12月碳源单耗达到半年峰值（157.57 kg/万m³），是低温对反硝化效率抑制的直接结果。随着水温回升至22°C以上，反硝化速率恢复，预计4–6月碳源单耗回落至100–120 kg/万m³区间，接近秋季（10月105.64）水平，降幅约25%–35%。

**（3）梅雨季高水量对年化碳排放估算的修正**

若4–6月月均处理水量达到19,000–21,000万m³（高于现有6个月均值17,217万m³），则9个月实测期内的月均碳排放将低于6个月推算值，年化碳排放可能修正为**98–103万tCO₂eq**（低于现有"105.66万tCO₂eq"估算约2%–7%）。这一修正将使年化碳排放估算更为保守（偏低），减少对碳汇需求的高估。

**（4）高温区N₂O排放因子验证**

M2模型中好氧路径A的N₂O排放因子（EF_nit）随温度增加而增大。深圳6月水温约27°C时，M2-nit的温度修正将使N₂O排放因子较冬季（15°C）高出约45%（$\theta_T^{27-15}=1.04^{12}\approx1.60$，实际因活性饱和效应约1.3–1.5倍）。这一温度区间是模型外推边界附近，4–6月的实测药耗和电耗数据有助于约束EF_nit在高温区的不确定性。

#### 7.7.8.3 数据纳入的操作方案

**步骤1**：将含4–6月月度统计数据的文件（推荐使用与现有报告相同格式的.docx文件）放入 `data/raw/`目录。

**步骤2**：从文件中提取以下月度指标，填入 `notebooks/extend_to_june_2026.py` 中的 `APRIL_JUNE_DATA` 列表：
- 处理水量（万m³）、用电量（kWh）
- 碳源单耗、除磷药剂单耗、脱水PAC单耗（均为kg/万m³）
...
**步骤4**：对比9个月与6个月分析结果，检验上述预期发现，更新本节7.7.3–7.7.6的相关数字。

---

*本节版本：v2.0 | 更新日期：2026-07-21 | Phase 9 新增：7.7.8节（数据扩展计划）、扩展预期新发现、操作方案；方法论注：月度单位碳排放使用年化日均流量口径（2月偏差约-7.9%，6月均值无误差）*
