...
## [v6.0.0] — 2026-07-21 （Phase 9：数据扩展至2026年6月 + 季节性发现）

### 新增（核心）
- **`notebooks/run_fpcm_shenzhen_2026jun.py`**：数据更新分析脚本，将深圳实证数据扩展至2026年6月（9个月完整数据），从XLS文件提取实测水质并自动计算加权平均
- **`data/processed/fpcm_monthly_46plants_2026jun.csv`**：9个月碳排放核算结果（Oct 2025–Jun 2026），含电耗来源标注
- **`data/processed/fpcm_monthly_46plants_2026jun.json`**：同上，JSON格式，包含完整元数据
- **`docs/data_analysis_verification.md`**：数据分析过程可复现验证文档，包含逐步手工验证方法（Excel SUMPRODUCT公式、回归参数验证、Scope 2直接验算）

### 修改（论文）
- **`paper/chapters/chapter_07_case_study.md`**：7.7节全面更新至v2.0，纳入2026年4-6月数据和3项新发现
- **`CHANGELOG.md`**：新增本版本记录

### 关键新发现
1. **夏季单位碳排放比冬季低9.4%**：夏（Apr-Jun）均值0.4845 vs 冬（Nov-Feb）均值0.5350 kgCO₂/m³
2. **降雨稀释导致进水COD降幅39%**：1月334 mg/L → 6月204 mg/L，TN降幅35%（1月44.7→6月29.2）
3. **碳排放峰值月修正**：原10月（96,560 tCO₂）→ 修正为6月（108,398 tCO₂），+11.7%
4. **年化碳排放修正**：基于旱季6个月外推值105.7万tCO₂ → 基于9个月的年化值113.6万tCO₂（+7.5%）
5. **Jan-Mar 2026水质数据修正**：使用XLS实测值替换典型值后，碳排估算增加2.3%~6.9%

### 数据说明（可溯源性）
- 水量和水质（Jan-Jun 2026）：来自 `data/raw/02-11 2026年数据汇总（污水厂）(更新到6月）.xls`
- 电耗（Oct 2025–Mar 2026）：来自能耗药耗报告docx（实测）
- 电耗（Apr-Jun 2026）：基于温度-电耗线性回归估算（R²=0.72，不确定性±5%）
